...
$query=mysqli_query($conn,"Select * from news where news_title LIKE '%$searchq%'AND user_id='".$_SESSION['user_id']."'")or die("could not search!");
$count= mysqli_num_rows($query);
if($count==0){
	$output='There was no search results!';
}else{
	while($row=mysqli_fetch_array($query)){
		$image='<img src="data:image;base64,'.base64_encode($row['news_image']).'"" style="width:150px; height:130px;"';
		$title=$row['news_title'];
		$region=$row['news_region'];
		$date=$row['news_date'];
		$detail=$row['news_desc'];
$output .= '<tr>
                        <td>' .$image.'</td>
						<td>' .$title. '</td>
						<td>' .$region. '</td>
	...
	<body>
		...
		<section>
			<article>
				...
				<center>	
				<form action="newssearchaction.php" method="post" class="carian">
					<button onclick="document.location='staff_listnews.php'" value="Show All">Show All</button>
