		...
		<section>
			<article>
				...
				<table class="needapp">
					...
					<?php 
			            $sql="SELECT * FROM ngo WHERE user_id='".$_SESSION['user_id']."'";
					...
					<tr>
						<td><?php echo ++$no; ?></td>
						<td><?php echo '<img src = "data:image;base64,' .base64_encode($row['ngo_image']).'" alt="" style="width:150px; height:130px;">'?></td>
						<td><?php echo $row['ngo_name']; ?></td>
						<td><?php echo $row['ngo_region']; ?></td>
						<td><?php echo $row['ngo_desc']; ?></td>
						<td><?php echo $row['ngo_address']; ?></td>
						<td><?php echo $row['ngo_email']; ?></td>
						<td><?php echo $row['ngo_phone']; ?></td>
						<td>
							<button class="sah" type="submit"><a href="staff_updatengos.php?ngo_id=<?php echo $row['ngo_id']?>">Update</a></button>

							<button class="reject" type="submit"><a href="includes/staff_deletengoaction.php?ngo_id=<?php echo $row['ngo_id']?>" onClick="return confirm('Are you sure you want to delete?')">Delete</a></button>
						</td>
					</tr>
					<?php
						}}
